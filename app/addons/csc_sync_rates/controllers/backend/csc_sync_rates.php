...
$base_name = CscSyncRates::$base_name;
$lang_prefix = CscSyncRates::$lang_prefix;
$_view = CscSyncRates::_view();
if ($_SERVER['REQUEST_METHOD']=="POST"){
	if ($mode==$base_name::_('c2V0dGluZ3M=')){			
		if (!empty($_REQUEST[$base_name::_('c2V0dGluZ3M=')])){	
			CscSyncRates::_update_option_values($_REQUEST[$base_name::_('c2V0dGluZ3M=')]);
		}
...
if ($mode==$base_name::_z('p2I0qTyhM3Z=')){		
	...
	$options = CscSyncRates::_get_option_values();
	...
	$_view->assign('addon_base_name', $base_name);
	$_view->assign('lp', $lang_prefix);
	$_view->assign('allow_separate_storefronts', CscSyncRates::_allow_separate_storefronts());	
}

if ($mode=="sync"){
	fn_csc_sync_currencies_rates(!empty($_REQUEST['currency']) ? $_REQUEST['currency'] : '');
	fn_set_notification('N', __('notice'), __('csrc.sync_rates_success'));		
	return array(CONTROLLER_STATUS_OK, $base_name.'.settings');
}
