...
<div class="ty-tabs cm-j-tabs{if $track} cm-track{/if} clearfix">
    <!-- class="ty-tabs__list" -->
    <ul class="ty-accordion" {if $tabs_section}id="tabs_{$tabs_section}"{/if}>
    {foreach from=$navigation.tabs item=tab key=key name=tabs}
        ...
        <li id="{$key}" class="ui-accordion-header{if $tab.js} cm-js{elseif $tab.ajax} cm-js cm-ajax{/if}{if $key == $active_tab} ui-state-active{/if}"><a class="ty-tabs__a" {if $tab.href} href="{$tab.href|fn_url}"{/if}>{$tab.title}</a></li>
...
{if $_tabs}
<div class="cm-tabs-content ui-accordion-content clearfix" id="tabs_content">
