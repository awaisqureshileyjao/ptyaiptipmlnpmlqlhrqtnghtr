<div class="ty-sort-container">
   <div class="ty-sort-container-block"> 
...
<div class="ut2-sort-label">{__('layout_view')}:</div>
...
<a id="sw_elm_pagination_steps" class="ty-sort-dropdown__wrapper cm-combination">{__("per_page")}: {$pagination.items_per_page}<i class="ty-sort-dropdown__icon ty-icon-down-micro"></i></a>
    <ul id="elm_pagination_steps" class="ty-sort-dropdown__content cm-popup-box hidden">
        {foreach from=$product_steps item="step"}
        {if $step != $pagination.items_per_page}
            <li class="ty-sort-dropdown__content-item">
                <a class="{$ajax_class} ty-sort-dropdown__content-item-a" href="{"`$range_url`&items_per_page=`$step`"|fn_url}" data-ca-target-id="{$pagination_id}" rel="nofollow">{$step}</a>
            </li>
        {/if}
        {/foreach}
    </ul>
...
{/if}
<div class="filter_display_btn">
<a class="filter_btn">Filter:<img class="filterbtn_img" src="design/themes/thedelamode/media/images/Vector.svg"/></a>
</div>    
</div>
</div>
