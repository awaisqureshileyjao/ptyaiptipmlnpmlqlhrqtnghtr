...
{*if $po.option_id == 26}
<div class="ty-control-group ty-product-opt">
{foreach from=$po.variants item=pov}
<div class="size_variant" data-val="{$pov.variant_id}" data-target="option_{$obj_prefix}{$id}_{$po.option_id}">{$pov.variant_name}</div>
{/foreach}
<div class="size guide_title">{__("size_guide")}</div>
</div>
{literal}<script>
$('.ty-product-block__button').addClass('size_option');
</script>{/literal}
{/if*}
