...
{capture name="login"}
    <form name="{$id}_form" action="{""|fn_url}" method="post" {if $style == "popup"}class="cm-ajax cm-ajax-full-render"{/if}>
        ...
        {if $auth.user_id == 0}<p>{__("have_an_account_login")}</p>{/if}
        ...
        {if $style == "popup"}
            ...
            {*<div class="ty-login-reglink ty-center">
                {__("donot_have_an_account")}<a class="ty-login-reglink__a" href="{"profiles.add"|fn_url}" rel="nofollow">{__("register_new_account")}</a>
            </div>*}
        {/if}
        ...
        {/hook}
        <div class="ty-login-reglink ty-center">
            {__("donot_have_an_account")}<a class="ty-login-reglink__a" href="{"profiles.add"|fn_url}" rel="nofollow">{__("register")}</a>
        </div>
    </form>
{/capture}
