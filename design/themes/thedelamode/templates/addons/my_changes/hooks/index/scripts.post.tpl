...
<script type="text/javascript">
		...
		$(document).on('click','.ty-product-detail .ty-product-block__img-wrapper .ty-product-img', function(e){
		...
		var s_stn_del_date  = DateHelper.format(DateHelper.addDays(new Date(), 5));
		var e_stn_del_date  = DateHelper.format(DateHelper.addDays(new Date(), 7));
		var s_exp_del_date = DateHelper.format(DateHelper.addDays(new Date(), 2));
		var e_exp_del_date = DateHelper.format(DateHelper.addDays(new Date(), 4));



		$('.mydesh_b .ty-account .ty-gdpr-agreement label').click(function(){
			$('.mydesh_b .ty-account .ty-gdpr-agreement input').trigger('click');
		});

	$(document).ready(function(){
				$('.stn_del_tim').html("will approximately from 5 to 7 working days <b>" + s_stn_del_date + '-' + e_stn_del_date + "</b>");
				$('.exp_del_tim').html(" will approximately 2 to 4 days. <b>" + s_exp_del_date + '-' + e_exp_del_date +"</b>");	

			$(document).on('click','.ty-product-options__radio--label', function(){
          var x = $(this).offset();
          var top = x.top; 
          ...
          setTimeout(function(){
            window.location.reload();
             $('html, body').animate({
                scrollTop: $(".search_result_"+top).offset() ? 
                 $(".search_result_"+top).offset().top - 300 : null
           }, 1500);
          },1200);
        }); 

//$(document).on('click','.thedelamode_register .ty-account .ty-gdpr-agreement label', function(){
	$('.thedelamode_register .ty-account .ty-gdpr-agreement label').click(function(){
		$('.ty-account .ty-gdpr-agreement input').trigger('click');
});



$(document).on('click','.ty-product-block__button.option_avail .ty-btn__tertiary .fa-heart-o', function(e){
	//e.preventDefault();
	$(this).removeClass('fa-heart-o').addClass('fa-heart');	
});

$(document).on('click','.ty-variations-list__wrapper .ty-btn__tertiary .fa-heart-o', function(e){
	//e.preventDefault();
	$(this).removeClass('fa-heart-o').addClass('fa-heart');	
});

$(document).on('hover','.ty-btn__tertiary .fa-heart-o', function(){
$(this).parent().prop('tooltipText', 'Add to wish list'); 
});
		...
		if($(window).width() < 980 ) {
			...
			$('.menu-opener').click(function(){
					...
					$('body').addClass('delamode');						
				});	
			$('.close_menu').click(function(){
				$('body').removeClass('delamode');		
			});	
...
$(window).scroll(function(){
  var sticky = $('.tygh-header.clearfix'),
      scroll = $(window).scrollTop();

  if (scroll >= 70) {
		  sticky.addClass('fixed');
  		$('.header_menu_section .ty-menu__items .ty-menu__item-active').removeClass('ty-menu__item-active');
  	} else { sticky.removeClass('fixed');
  		$('.header_menu_section .ty-menu__items li:nth-child(2)').addClass('ty-menu__item-active');
  		$(document).on('hover', '.header_menu_section .ty-menu__items .ty-menu__item', function(){  			
...
$(document).on('click','.cat_show .overlap_cat',function(){
	$(this).parents().find('.homecustompage').removeClass('cat_show');
	$(this).parents().find('.category_filters').removeClass('show');
});

$(document).on('click','.thedelamode-cat .filter_btn',function(){
	$(this).parents().find('.homecustompage').addClass('cat_show');
	$(this).parents().find('.category_filters').addClass('show');
});
 ...
 /* $(".search_result_"+letter)
  ...
  $('html, body').animate({
        scrollTop: $(".search_result_"+letter).offset() ? 
         $(".search_result_"+letter).offset().top - 330 : null
   }, 2000);
