    ...
    <form action="{""|fn_url}" method="post" name="subscribe_form" class="cm-processing-personal-data">
        ...
        <div class="ty-footer-form-block__form ty-control-group">
            ...
            <label class="cm-required cm-email hidden" for="subscr_email{$block.block_id}">{__("email")}</label>
                <input type="text" name="subscribe_email" id="subscr_email__{$block.block_id}" size="20" placeholder="{__("email")}*" class="ty-input-text-medium ty-valign-top" />

                <a data-ca-target-id="suscribe_terms" id="opener_suscribe_terms" class="cm-dialog-opener" href="{"products.newsletter"|fn_url}" title="{__("subscription_conditions")}">Terms & condition</a>
