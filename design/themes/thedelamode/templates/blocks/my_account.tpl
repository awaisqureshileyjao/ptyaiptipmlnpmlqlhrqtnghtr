...
<div id="account_info_{$block.snapping_id}">
    ...
    <ul class="ty-account-info">
        {hook name="profiles:my_account_menu"}
            {capture name="title"}
                ...
                {if $auth.user_id}
                    {if $user_info.firstname || $user_info.lastname}
                       {__("hello")} {$user_info.firstname} {*$user_info.lastname*}
                    {else}
                        {$user_info.email}
                    {/if}
                {else}
            ...
            {if $auth.user_id}
                ...
                {/if*}
                <li class="ty-account-info__item ty-dropdown-box__item"><a class="ty-account-info__a underlined" href="{"profiles.update&type=p"|fn_url}" rel="nofollow" >{__("my_dashboard")}</a></li>
    ...
    <div class="ty-account-info__buttons buttons-container">
        ...
        {else}
            {*<a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}" data-ca-target-id="login_block{$block.snapping_id}" class="cm-dialog-opener cm-dialog-auto-size ty-btn ty-btn__secondary thedelamode_login_btn" data-ca-dialog-title="{__("registered_customers")}" rel="nofollow">{__("sign_in")}</a>*}

            {*<a href="{"profiles.add"|fn_url}" class="cm-dialog-opener cm-dialog-auto-size ty-btn ty-btn__primary thedelamode_register_btn"  data-ca-target-id="user_registration{$block.snapping_id}" data-ca-dialog-title="{__("create_account")}" >{__("register")}</a>*}
            
            <a href="{if $runtime.controller == "auth" && $runtime.mode == "login_form"}{$config.current_url|fn_url}{else}{"auth.login_form?return_url=`$return_current_url`"|fn_url}{/if}" class="ty-btn ty-btn__secondary thedelamode_login_btn"  rel="nofollow">{__("sign_in")}</a>

            <a href="{"profiles.add"|fn_url}" rel="nofollow" class="ty-btn ty-btn__primary ">{__("register")}</a>

            <div  id="login_block{$block.snapping_id}" class="hidden thedelamode_login" title="{__("sign_in")}">
                <div class="ty-login-popup">
                    {include file="views/auth/login_form.tpl" style="popup" id="popup`$block.snapping_id`"}
                </div>
            </div>
          
            <a  data-ca-target-id="page_tl_6" id="opener_page_tl_6" class="cm-dialog-opener thedelamode_register " href="{"profiles.add"|fn_url}" title="{__("register")}"}></a>
