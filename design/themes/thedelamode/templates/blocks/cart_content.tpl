    ...
    <div class="ty-dropdown-box" id="cart_status_{$dropdown_id}">
        ...
        <a href="{"checkout.cart"|fn_url}">
            ...
            <span class="ty-minicart-title{if !$smarty.session.cart.amount} empty-cart{/if} ty-hand">
                <span class="ty-block ty-minicart-title__header ty-uppercase">{*__("my_cart")*}{include file="common/price.tpl" value=$smarty.session.cart.display_subtotal}</span>
                <span class="ty-block">
                ...
                {else}
                    <span>{__("cart_amount_empty")}</span>
                {/if}
                </span>
            </span>
            {/hook}
